...
def send_all_fvs_to_server(all_fvs, user_id="unknown"):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            # RDF Integration: Handle authenticated RDF profile
            if response_obj.get("final_decision") == "granted" and "rdf_profile" in response_obj:
                if response_obj["rdf_profile"] is not None:
                    matched_user = next(r["matched_user"] for r in response_obj["results"] if r["status"] == "granted")
                    rdf_save_path = f"{matched_user}_profile.ttl"
                    
                    with open(rdf_save_path, "w", encoding="utf-8") as f:
                        f.write(response_obj["rdf_profile"])
                    
                    print(f"\n🔓 SUCCESS: Authenticated access granted!")
                    print(f"📄 Your RDF WebID profile has been saved to: {rdf_save_path}")
                    print("=" * 70)
                    print(response_obj["rdf_profile"])
                    print("=" * 70)
                else:
                    print("\n✅ Authenticated, but no RDF profile exists for this user yet.")
